...
def run_segmentation(frame, session, input_name, output_name, gpu_clahe_output, gpu_input_float, gpu_resize):
    ...
    clahe = cv2.createCLAHE(clipLimit=1.5, tileGridSize=(8, 8))
    frame_gray_c = clahe.apply(np.uint8(frame_gray_resized_cpu))

    # Normalize and shape for the model
...
if __name__ == "__main__":
    ...
    while g_running:
        try: frame_package = frame_queue.get(timeout=0.1) 
        ...
        # --- segmentation ---
        predict_map, img_bgr = run_segmentation(
