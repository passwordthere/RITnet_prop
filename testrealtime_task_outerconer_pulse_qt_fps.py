...
def draw_target_crosshair(frame, predict, frame_id, fps, offset_x=20, offset_y=20):
    frame_with_guide = frame.copy()

    # --- [新增] 1. 绘制 FPS ---
    fps_text = f"FPS: {fps:.1f}"
    # (图像, 文本, 坐标(x,y), 字体, 缩放, 颜色BGR, 粗细)
    cv2.putText(frame_with_guide, fps_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
...
def run_realtime_inference(model, device, transform, sock: socket.socket):
    ...
    # --- [新增] FPS 计算变量 ---
    frame_times = deque(maxlen=30) # 存储最近 30 帧的处理时间
    fps = 0.0                      # 当前计算出的FPS
    last_print_time = time.time()  # 用于控制台打印的时间戳
    # --------------------------
    ...
    with torch.no_grad():
        while True:  # 无限循环
            for img_path in image_files:
                ...
                display_frame = draw_target_crosshair(frame_resized,  pred_cpu, frame_id, fps, offset_x=20,  offset_y=20)
                frame_id += 1
                ...
                try:
                    ...
                    # --- [新增] FPS 计算和打印逻辑 ---
                    # 1. 计算此帧耗时并更新列表
                    loop_time_secs = time.time() - loop_start_time
                    frame_times.append(loop_time_secs)
